...
| `--agent <name>` | | 以指定 Agent 作为主 Agent 启动会话（仅 `kimi -p`，两个引擎均支持） |
| `--agent-file <path>` | | 从 Markdown 文件加载自定义 Agent（仅本次启动、仅 `kimi -p`，两个引擎均支持）并选中它。不可重复传入，也不能与 `--agent` 同时使用 |
...
`--agent` 和 `--agent-file` 用于选择驱动会话的 Agent。二者在两个引擎的 `kimi -p` 下均可用，交互式启动会以明确错误拒绝：
