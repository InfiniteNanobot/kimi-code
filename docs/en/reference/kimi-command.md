...
| `--agent <name>` | | Start the session with the specified agent as the main Agent (`kimi -p` only, on either engine) |
| `--agent-file <path>` | | Load a custom agent from a Markdown file for this launch and select it (`kimi -p` only, on either engine). Cannot be repeated or combined with `--agent` |
...
`--agent` and `--agent-file` select which agent drives the session. Both are available under `kimi -p` on either engine; interactive launches reject them with a clear error:
